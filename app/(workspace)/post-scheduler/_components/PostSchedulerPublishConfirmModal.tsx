...
interface PostSchedulerPublishConfirmModalProps {
...
export function PostSchedulerPublishConfirmModal({
  visible,
  channelCount,
  targets,
  onCancel,
  onConfirm,
}: PostSchedulerPublishConfirmModalProps): React.ReactElement | null {
      ...
      <div className="pointer-events-none fixed left-1/2 top-1/2 w-full max-w-lg -translate-x-1/2 -translate-y-1/2 px-5">
        ...
        >
          <h2 className="font-headline text-lg font-bold text-on-surface">
            {t("postScheduler.publish.publishNowTitle")}
          </h2>
          <p className="mt-3 font-body text-sm leading-relaxed text-on-surface-variant">
            {t("postScheduler.publish.publishBody", { count: channelCount })}
          </p>
          {detailRows.length > 0 ? (
