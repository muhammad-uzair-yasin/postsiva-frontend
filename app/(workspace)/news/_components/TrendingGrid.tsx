...
import type { TrendingPostItem } from "@/lib/news/trendingApi";
...
export function TrendingGrid({
  ...
  error,
  onLoadMore,
}: TrendingGridProps): React.ReactElement {
  ...
  if (posts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-sm text-white/50">No trending posts found</p>
        <p className="mt-1 text-xs text-white/30">Try a different country or category</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <p className="text-xs text-on-surface-variant">
        {total.toLocaleString()} trending posts
      </p>
