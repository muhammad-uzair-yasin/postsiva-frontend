...
interface NewsCreatePostModalProps {
  article: ArticleItem;
  onClose: () => void;
  /** Defaults to news; RSS tab reuses the same modal. */
  source?: "news" | "rss";
}

export function NewsCreatePostModal({
  article,
  onClose,
  source = "news",
}: NewsCreatePostModalProps): React.ReactElement {
  ...
  function handleCreate() {
    ...
    const media = mediaFromArticleImageUrl(article.image);
    ...
    >
      ...
      >
        ...
        <div className="border-b border-outline-variant/10 bg-surface-container-low px-6 py-3">
          ...
          {article.source ? (
            <p className="mt-0.5 text-xs text-primary">{article.source}</p>
          ) : null}
          <p className="mt-1 text-[11px] text-on-surface-variant">
            Opens the post composer and drafts a caption from this article.
          </p>
        </div>
