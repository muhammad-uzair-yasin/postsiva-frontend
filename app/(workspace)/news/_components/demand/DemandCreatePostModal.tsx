...
import { setDemandComposerHandoff } from "@/lib/news/demandComposerHandoff";
...
interface DemandCreatePostModalProps {
  payload: DemandCreatePayload;
  onClose: () => void;
}

export function DemandCreatePostModal({
  payload,
  onClose,
}: DemandCreatePostModalProps): React.ReactElement {
  ...
  function handleCreate() {
    ...
    const media = mediaFromArticleImageUrl(payload.image_url);
  ...
  const modeLabel =
    payload.source_type === "rising"
      ? "Rising search"
      : payload.source_type === "topic"
        ? "Topic suggestion"
        : "Culture pulse";
    ...
    >
      ...
      >
        ...
        <div className="border-b border-outline-variant/10 bg-surface-container-low px-6 py-3">
          ...
          <p className="mt-0.5 text-xs text-primary">{modeLabel}</p>
          <p className="mt-1 text-[11px] text-on-surface-variant">
            Opens the post composer and drafts a caption inspired by this demand
            signal.
          </p>
        </div>
