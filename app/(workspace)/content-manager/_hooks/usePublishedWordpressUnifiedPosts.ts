...
import { wordpressConnectionIdFromHeaderAccountId } from "@/lib/social/unifiedBlogPlatform";
...
import { mapUnifiedWordpressToContentManagerPosts } from "../_utils/mapUnifiedWordpressPosts";
...
export function usePublishedWordpressUnifiedPosts(
  wordpressHandleLabel: string,
    ...
    async (kind: LoadKind): Promise<void> => {
      ...
      try {
        const connectionId = selectedAccount?.id
          ? wordpressConnectionIdFromHeaderAccountId(selectedAccount.id) ?? undefined
          : undefined;
        ...
        if (!data.success) {
          ...
          setError(data.message ?? "Could not load WordPress posts");
          return;
        }
        const mapped = mapUnifiedWordpressToContentManagerPosts(
          data.wordpress ?? null,
          wordpressHandleLabel,
        );
        if (isRefresh) {
        ...
        setError(e instanceof Error ? e.message : "Could not load WordPress posts");
      } finally {
        setIsLoading(false);
      }
    },
    [limit, selectedAccount?.id, skip, token, wordpressHandleLabel, workspaceId],
  );
  ...
  useEffect(() => {
    ...
    if (typeof decision === "object") {
      setPublishedPostsIfChanged(setPosts, filterPublishedCachePosts(decision.cache, "wordpress"));
      setIsLoading(false);
      setError(null);
      return;
    }
