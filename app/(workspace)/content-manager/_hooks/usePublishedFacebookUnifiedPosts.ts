...
import { mapUnifiedFacebookToContentManagerPosts } from "../_utils/mapUnifiedFacebookPosts";
...
export function usePublishedFacebookUnifiedPosts(
  facebookHandleLabel: string,
    ...
    async (kind: LoadKind): Promise<void> => {
      ...
      try {
        const data = await fetchUnifiedPosts(token, workspaceId, {
          ...
          facebookPageIds: facebookPageId ? [facebookPageId] : [],
        });
        if (!data.success) {
          if (!isRefresh) {
            setPosts([]);
          }
          setError(data.message ?? "Could not load Facebook posts");
          return;
        }
        ...
        setError(e instanceof Error ? e.message : "Could not load Facebook posts");
      } finally {
        setIsLoading(false);
      }
    },
    [facebookHandleLabel, facebookPageId, limit, skip, token, workspaceId],
  );
  ...
  useEffect(() => {
    const channelFilter = facebookPageId ? `facebook:${facebookPageId}` : "facebook";
    ...
    if (typeof decision === "object") {
      ...
      // Page-scoped cache can be hydrated with the wrong filter shape (handle vs
      // page id). If nothing matches this page, fall through to a real fetch.
      if (fromCache.length > 0 || !facebookPageId) {
  ...
  }, [facebookPageId, isLoadingProfiles, limit, publishedCacheVersion, selectedAccount?.id, skip, token, workspaceId]);
