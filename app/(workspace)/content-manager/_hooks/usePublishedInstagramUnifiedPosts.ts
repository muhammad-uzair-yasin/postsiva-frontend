...
import { mapUnifiedInstagramToContentManagerPosts } from "../_utils/mapUnifiedInstagramPosts";
...
export function usePublishedInstagramUnifiedPosts(
    ...
    async (kind: LoadKind): Promise<void> => {
      ...
      try {
        ...
        if (!data.success) {
          ...
          setError(data.message ?? "Could not load Instagram posts");
          return;
        }
        ...
        setError(e instanceof Error ? e.message : "Could not load Instagram posts");
      } finally {
        setIsLoading(false);
      }
    },
    [instagramHandleLabel, limit, skip, token, workspaceId],
  );
  ...
  useEffect(() => {
    ...
    if (typeof decision === "object") {
      setPublishedPostsIfChanged(setPosts, filterPublishedCachePosts(decision.cache, "instagram"));
      setIsLoading(false);
      setError(null);
      return;
    }
