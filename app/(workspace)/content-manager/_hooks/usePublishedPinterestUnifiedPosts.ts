...
import { mapUnifiedPinterestToContentManagerPosts } from "../_utils/mapUnifiedPinterestPosts";
...
export function usePublishedPinterestUnifiedPosts(
  pinterestHandleLabel: string,
    ...
    async (kind: LoadKind): Promise<void> => {
      ...
      try {
        ...
        if (!data.success) {
          ...
          setError(data.message ?? "Could not load Pinterest pins");
          return;
        }
        const mapped = mapUnifiedPinterestToContentManagerPosts(
          data.pinterest ?? null,
          pinterestHandleLabel,
        );
        if (isRefresh) {
        ...
        setError(e instanceof Error ? e.message : "Could not load Pinterest pins");
      } finally {
        setIsLoading(false);
      }
    },
    [limit, pinterestHandleLabel, skip, token, workspaceId],
  );
  ...
  useEffect(() => {
    ...
    if (typeof decision === "object") {
      setPublishedPostsIfChanged(setPosts, filterPublishedCachePosts(decision.cache, "pinterest"));
      setIsLoading(false);
      setError(null);
      return;
    }
