...
import { mapUnifiedMastodonToContentManagerPosts } from "../_utils/mapUnifiedMastodonPosts";
...
export function usePublishedMastodonUnifiedPosts(
  mastodonHandleLabel: string,
    ...
    async (kind: LoadKind): Promise<void> => {
      ...
      try {
        ...
        if (!data.success) {
          ...
          setError(data.message ?? "Could not load Mastodon posts");
          return;
        }
        const mapped = mapUnifiedMastodonToContentManagerPosts(
          data.mastodon ?? null,
          mastodonHandleLabel,
        );
        if (isRefresh) {
        ...
        setError(e instanceof Error ? e.message : "Could not load Mastodon posts");
      } finally {
        setIsLoading(false);
      }
    },
    [limit, mastodonHandleLabel, skip, token, workspaceId],
  );
  ...
  useEffect(() => {
    ...
    if (typeof decision === "object") {
      setPublishedPostsIfChanged(setPosts, filterPublishedCachePosts(decision.cache, "mastodon"));
      setIsLoading(false);
      setError(null);
      return;
    }
