...
import { mapUnifiedBlueskyToContentManagerPosts } from "../_utils/mapUnifiedBlueskyPosts";
...
export function usePublishedBlueskyUnifiedPosts(
  blueskyHandleLabel: string,
    ...
    async (kind: LoadKind): Promise<void> => {
      ...
      try {
        ...
        if (!data.success) {
          ...
          setError(data.message ?? "Could not load Bluesky posts");
          return;
        }
        ...
        setError(e instanceof Error ? e.message : "Could not load Bluesky posts");
      } finally {
        setIsLoading(false);
      }
    },
    [blueskyHandleLabel, limit, skip, token, workspaceId],
  );
  ...
  useEffect(() => {
    ...
    if (typeof decision === "object") {
      setPublishedPostsIfChanged(setPosts, filterPublishedCachePosts(decision.cache, "bluesky"));
      setIsLoading(false);
      setError(null);
      return;
    }
