...
import { useFacebookOAuthComplete } from "../_hooks/useFacebookOAuthComplete";

export function FacebookOAuthCompleteScreen(): React.ReactElement {
  const { phase, message } = useFacebookOAuthComplete();

  if (phase === "loading") {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-surface px-6 text-on-surface">
        <p className="text-lg font-semibold">Completing Facebook sign-in…</p>
