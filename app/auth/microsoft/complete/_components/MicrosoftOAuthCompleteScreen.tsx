...
import { useMicrosoftOAuthComplete } from "../_hooks/useMicrosoftOAuthComplete";

export function MicrosoftOAuthCompleteScreen(): React.ReactElement {
  const { phase, message } = useMicrosoftOAuthComplete();

  if (phase === "loading") {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-surface px-6 text-on-surface">
        <p className="text-lg font-semibold">Completing Microsoft sign-in…</p>
