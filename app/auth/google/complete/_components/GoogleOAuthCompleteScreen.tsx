...
import { useGoogleOAuthComplete } from "../_hooks/useGoogleOAuthComplete";

export function GoogleOAuthCompleteScreen(): React.ReactElement {
  const { phase, message } = useGoogleOAuthComplete();

  if (phase === "loading") {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-surface px-6 text-on-surface">
        <p className="text-lg font-semibold">Completing Google sign-in…</p>
