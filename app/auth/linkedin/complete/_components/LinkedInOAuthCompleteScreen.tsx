...
import { useLinkedInOAuthComplete } from "../_hooks/useLinkedInOAuthComplete";

export function LinkedInOAuthCompleteScreen(): React.ReactElement {
  const { phase, message } = useLinkedInOAuthComplete();

  if (phase === "loading") {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-surface px-6 text-on-surface">
        <p className="text-lg font-semibold">Completing LinkedIn sign-in…</p>
